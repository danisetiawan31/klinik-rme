...
func TestKlinikAntrianClaimEndpoints_Integration(t *testing.T) {
	...
	t.Run("Pure DB Layer Concurrency Claim (FOR UPDATE SKIP LOCKED)", func(t *testing.T) {
		...
		assert.Less(t, dbParDuration, 2*dbSingleDuration, "Parallel DB duration for 5 claims must be close to single claim duration (< 2x single claim duration) when MaxConns=20 is configured")
	})
}
