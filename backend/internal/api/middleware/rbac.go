...
func RequireRole(allowedRoles ...string) gin.HandlerFunc {
	return func(c *gin.Context) {
		// auth_channel="display-token" HANYA pernah di-set oleh middleware DualAuth
		// (dipakai GET /klinik/:id/antrian dan nanti WS /ws) — bypass ini TIDAK PERNAH
		// ter-trigger untuk endpoint lain yang pakai Authenticate biasa.
		if channel, ok := c.Get("auth_channel"); ok && channel == "display-token" {
			c.Next()
			return
		}
