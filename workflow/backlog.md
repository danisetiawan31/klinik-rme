...
- [ ] **6. Realtime & Papan Antrian** (Tahap 1 Selesai) _(swappable dengan 7 — cuma gantung ke item 5)_ — migration `klinik.display_token_hash`; Hub WS in-memory (`gorilla/websocket`); `POST /admin/klinik/:id/display-token/regenerate`; middleware dual-auth (cookie | `X-Display-Token`); broadcast notify-then-refetch di tiap write relevan.
