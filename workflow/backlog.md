...
- [x] **18. Admin dashboard** (Selesai Penuh) — manajemen user (invite dengan modal Reveal Once token, **resend invite**, **edit email/nama**, ubah roles dengan validasi mutual exclusivity `admin` vs `dokter`), filter + detail audit log dengan visual `AuditDiffViewer` (before/after field diff & tamper-evident SHA-256 hash), regenerate display token dengan dialog konfirmasi dampak dan Reveal Once Secret.
