...
| `ToastNotification` / `HlmToaster`             | Primitive / Composed | Toast top-right, programmatic `toast.error/success/info`, ARIA live region (§9.5)   | Spartan Sonner (`ngx-sonner`)                          | Lintas Fitur                        | Selesai           |
...
| `RevealOnceSecret`                              | Composed  | Tampilkan token/link sekali + copy (§9.2)                                                                 | Spartan Input (readonly), Button                       | Admin (invite, display-token)       | Selesai (Tahap 2 Admin) |
| `DataTable`                                     | Composed  | Tabel + pagination bernomor + collapse ke card (§9.3)                                                     | Spartan Table                                          | Pasien, Antrian, Admin              | Direncanakan      |
| `StatusBadge`                                   | Composed  | Badge status antrian (§9.4)                                                                               | Spartan Badge                                          | Antrian                             | Selesai (Tahap 1 Antrian) |
| `PriorityBadge`                                 | Composed  | Badge prioritas, ikon+label (§9.4)                                                                        | Spartan Badge                                          | Antrian, Pasien                     | Selesai (Tahap 1 Antrian) |
| `ClinicStatusIndicator`                         | Composed  | Badge buka/tutup klinik (§9.4)                                                                            | Spartan Badge                                          | App shell/header staff              | Selesai           |
| `ConfirmDialog` / `AntrianDaftarDialog`         | Composed  | Dialog konfirmasi Tidak Hadir & pendaftaran antrian (§9.7), CDK overlay + focus trap + backdrop dismiss   | Spartan Dialog (`HlmDialog`, `HlmDialogContent`, `HlmDialogHeader`, `HlmDialogFooter`) | Antrian, Pasien Detail           | Selesai           |
| `DiagnosisTindakanFormArray`                    | Composed  | Repeatable row group add/remove (§9.6) diagnosis ICD-10 & resep/tindakan                                  | Spartan Input, Select, Button + `FormArray`            | Rekam Medis                         | Selesai (Tahap 2-3) |
| `RekamMedisList`                                | Composed  | Dashboard rekam medis klinis dokter (hero pasien dipanggil, live search pasien/NIK, tab antrian & rekam medis selesai) | PriorityBadge, StatusBadge, Spartan Input, Button, Skeleton | Rekam Medis (/rekam-medis)          | Selesai           |
| `RekamMedisForm`                                | Composed  | Formulir pemeriksaan SOAP klinis dokter, FormArray diagnosis/resep, header antrian & riwayat drawer       | SensitiveValue, PriorityBadge, Spartan Form Controls   | Rekam Medis (/rekam-medis/pemeriksaan/:id) | Selesai (Tahap 2) |
| `SoapPatientHeader`                            | Composed  | Strip identitas pasien konteks pemeriksaan & panel collapsible riwayat rekam medis terdahulu               | SensitiveValue, PriorityBadge, Spartan Badge, Button   | RekamMedisForm                      | Selesai (P3)      |
| `SoapSubjectiveSection`                        | Composed  | Section S (Anamnesis / Keluhan & Gejala Pasien) dengan visual initial badge S mono                        | Spartan Card, Label, Textarea                          | RekamMedisForm                      | Selesai (P3)      |
| `SoapObjectiveSection`                         | Composed  | Section O (Pemeriksaan Fisik & Tanda Vital Objektif) dengan visual initial badge O mono                    | Spartan Card, Label, Textarea                          | RekamMedisForm                      | Selesai (P3)      |
| `SoapAssessmentSection`                        | Composed  | Section A (Diagnosis Klinis ICD-10) dynamic rows FormArray dengan initial badge A mono                    | Spartan Card, Input, Button                            | RekamMedisForm                      | Selesai (P3)      |
| `SoapPlanSection`                              | Composed  | Section P (Tindakan Medis & Resep Obat) dynamic rows FormArray dengan initial badge P mono & empty state   | Spartan Card, Input, Button, Native Select             | RekamMedisForm                      | Selesai (P3)      |
| `RekamMedisDetail`                              | Composed  | Tampilan leaf rekam medis, addendum notice banner, modal koreksi addendum berantai (`HlmDialog`)          | Spartan Card, Table, Dialog, Badge, Alert             | Rekam Medis (/rekam-medis/kunjungan/:id) | Selesai (Tahap 3) |
| `PapanAntrian`                                  | Composed  | Layar display board publik ruang tunggu TV/landscape, nomor dominan 3-digit zero-padded, status panggilan  | ConnectionStatusIndicator, PriorityBadge, Spartan Card | Papan Antrian (/papan-antrian)      | Selesai (Tahap 2-3) |
| `AuditDiffViewer`                               | Composed  | Diff view before/after JSON (§9.9)                                                                        | — (custom)                                             | Admin > Audit Log                   | Selesai (Tahap 3 Admin) |
| `AdminDashboard`                                | Composed  | Dashboard administrasi klinik terpadu (tab: Manajemen Pengguna, Jejak Audit, Pengaturan Display Token)    | AdminUsers, AdminAuditLog, AdminKlinik, RevealOnceSecret | Admin (/admin)                      | Selesai (Tahap 2-4 Admin) |
| `LandingHero`                                   | Composed  | Hero banner adaptif multi-role dengan live Jakarta time & operational status indicator                   | Spartan Badge, Lucide Icon                             | Beranda (Landing)                   | Selesai           |
| `LandingKpiCard`                                | Composed  | Kartu metrik KPI beranda dengan 5 varian warna semantik (primary, emerald, amber, purple, sky)           | Lucide Icon                                            | Beranda (Landing)                   | Selesai           |
| `DoctorDashboard`                               | Composed  | Workspace klinis dokter: spotlight antrian aktif, tab antrian periksa/selesai, quick EMR search          | LandingHero, LandingKpiCard, PriorityBadge             | Beranda (/ - Dokter)                | Selesai           |
| `PetugasDashboard`                              | Composed  | Triage & loket pendaftaran: registrasi cepat, live queue feed, panduan prioritas triage                  | LandingHero, LandingKpiCard, StatusBadge, PriorityBadge| Beranda (/ - Petugas)               | Selesai           |
| `AdminDashboardView`                            | Composed  | Pusat kendali administrasi & tata kelola: governance shortcuts, live antrian, ApexCharts radial gauge    | LandingHero, LandingKpiCard, NgApexcharts              | Beranda (/ - Admin)                 | Selesai           |
| `ConnectionStatusIndicator`                     | Composed  | Indikator status WS (§9.10)                                                                               | Spartan Badge                                          | Papan Antrian, widget antrian staff | Selesai           |
| Button / Badge / Avatar / Dropdown-Menu / Sheet / Sidebar / Dialog / Card / Table / Skeleton / Input / Label / Textarea / Alert / Empty / **Icon** | Primitive | Primitive dasar Spartan (diverifikasi §10)                                                                | —                                                      | Lintas fitur                        | Selesai           |
